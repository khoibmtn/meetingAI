...
/** Xoá bản ghi (chỉ chủ sở hữu). Tệp trên Drive được chuyển vào Thùng rác (khôi phục được 30 ngày). */
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    const { supabase, user } = await requireApiUser();
    const { data: recording } = await supabase.from("recordings").select("id, owner_id, drive_file_id").eq("id", id).maybeSingle();
